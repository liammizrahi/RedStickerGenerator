...
class UILabelX: UILabel {
    ...
    func centreArcPerpendicular() {
        ...
        let characters: [String] = str.reversed().map { String($0) } // An array of single character strings, each character in str
    ...
    */
    func centre(text str: String, context: CGContext, radius r:CGFloat, angle theta: CGFloat, slantAngle: CGFloat) {
        ...
        let attributes = [NSAttributedString.Key.foregroundColor: self.textColor!,NSAttributedString.Key.font: self.font] as [NSAttributedString.Key : Any]
        // Save the context
        context.saveGState()
